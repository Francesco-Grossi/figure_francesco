...
eval_sofun_byvar <- function(varnam,
  ...
  if ("fluxnet" %in% datasource) {
    ...
    ddf <- obs_eval$ddf %>%
      tidyr::unnest(data) %>%
      dplyr::select(sitename, date, obs = {{ varnam_obs }}, lat, igbp_land_use)
    xdf <- obs_eval$xdf %>%
      tidyr::unnest(data) %>%
    ...
    if (sum(!is.na(ddf$obs)) > 2) {
      ...
      if (!light) {
        ...
        meandoydf_byclim <- ddf %>%
          ...
          group_by(igbp_land_use, hemisphere, doy) %>%
          summarise(
          ...
          mutate(climatezone = paste(igbp_land_use, hemisphere))

        # meandoydf_byclim_stats <- meandoydf_byclim %>%
        #   group_by( igbp_land_use, hemisphere ) %>%
        #   nest() %>%
        #   mutate( n_obs  = purrr::map( data, ~sum(!is.na(.$obs_mean)))) %>%
        #   unnest( n_obs ) %>%
        #   dplyr::filter( n_obs>0, !is.na(igbp_land_use) ) %>%
        #   mutate(
...
extract_igbp_land_use <- function(str) {
