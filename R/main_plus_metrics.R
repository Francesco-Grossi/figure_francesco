      ...
      plot_modobs_spatial_annual <- function(lab) {
        ...
        gg <- df %>%
          ...
          labs(
            ...
            y = substitute(paste("Observed ",lab, "yr"^-1, ")"),list(lab=lab)),
            x = substitute(paste("Simulated ",lab,"yr"^-1, ")"),list(lab=lab))
          )
  ...
  if ("camels" %in% datasource) {
    ...
    if (!light) {
      ...
      plot_modobs_spatial_annual <- function(lab) {
        ...
        rsq_lab_annual <- round(metrics$annual_pooled$rsq, digits = 2)
        rmse_lab_annual <- round(metrics$annual_pooled$rmse, digits = 3)
        ...
        gg <- df %>%
          ...
          labs(
            subtitle =
              bquote(bold("Spatial:") ~ italic(R)^2 == .(rsq_lab_annual) ~ ~
                     RMSE == .(rmse_lab_annual)),
            y = substitute(paste("Observed AET (mm yr"^-1, ")"),list(lab=lab)),
            x = substitute(paste("Simulated AET (mm yr"^-1, ")"),list(lab=lab))
          ) 
      ...
      #             bquote(bold("Annual:") ~ italic(R)^2 == .(rsq_lab_annual) ~ ~
      #                      RMSE == .(rmse_lab_annual) ),
      #       x = substitute(paste("observed ",lab, "d"^-1, ")"),list(lab=lab)),
      #       y = substitute(paste("simulated ",lab, "d"^-1, ")"),list(lab=lab))
      #       )
      #   
      #   return(gg)
      # }
      ...
      plot_modobs_multi_annual <- function(lab) {
        ...
        # Round for display (still numeric)
        # rsq_lab_spatial <- round(rsq_lab_spatial, 2)
        # rmse_lab_spatial <- round(rmse_lab_spatial, 3)
        
        # Add labels
        gg <- modobs_adf$gg +
          labs(
            subtitle = bquote(
              bold("Multi year average:") ~ italic(R)^2 == .(rsq_lab_spatial) ~ RMSE == .(rmse_lab_spatial)
            ),
            x = substitute(paste("Observed AET (mm yr"^-1, ")"),list(lab=lab)),
            y =  substitute(paste("Simulated AET (mm yr"^-1, ")"),list(lab=lab))
          )
